...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define RAZOR_SERVER_HOSTNAME do |razor|
    razor.vm.box = "centos-64-x64-vbox4210-nocm"
    razor.vm.network "private_network", ip: "192.168.51.11"
    razor.vm.network "private_network", ip: RAZOR_SERVER_IP,
    ...
    razor.vm.network :forwarded_port,
      guest: 22,
      host: 60000,
      id: "ssh",
      auto_correct: true

    razor.vm.provision "shell", path: 'disable_iptables.sh'
  end

  config.vm.define PUPPET_SERVER_HOSTNAME do |puppet|
    puppet.vm.box = "centos-64-x64-vbox4210-nocm"
    puppet.vm.network "private_network", ip: "192.168.51.21"
    ...
    # Overwrite SSH port forwarding to prevent collisions when resuming from suspended mode
    puppet.vm.network :forwarded_port,
      guest: 22,
      host: 60001,
      id: "ssh",
      auto_correct: true

    puppet.vm.provision "shell", path: 'disable_iptables.sh' 
  end

  config.vm.define DHCP_SERVER_HOSTNAME do |dhcp|
    dhcp.vm.box = "centos-64-x64-vbox4210-nocm"
    dhcp.vm.network "private_network", ip: "192.168.51.31"
    dhcp.vm.network "private_network", ip: DHCP_SERVER_IP,
      virtualbox__intnet: INTERNAL_NETWORK_NAME
    ...
    # Overwrite SSH port forwarding to prevent collisions when resuming from suspended mode
    dhcp.vm.network :forwarded_port,
      guest: 22,
      host: 60002,
      id: "ssh",
      auto_correct: true 
   
    dhcp.vm.provision "shell", path: 'disable_iptables.sh' 
  end
